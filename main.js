var can_vPort = document.getElementById('viewport'),/* Viewport canvas */
    can_pCanvas = document.getElementById('player'),/* Player canvas */
    grid = { 'width': 60, 'height': 60 },/* This is in tiles not in pixels */
...
var ctx_vPort = can_vPort.getContext('2d'),/* Viewport canvas context */
    ctx_pCanvas = can_pCanvas.getContext('2d'),/* Player canvas context */
...
function Sprite(key) {
    ...
    this.update = function() {
        ...
        const dx = this.pos2.x - mouse.x; //diff between pointer and player
        const dy = this.pos2.y - mouse.y;
    console.log('dxdy',dx , dy);

        //     /* Here we check to see if any buttons where pressed and if our character is in the world */
            if (mouse.x != this.pos.x && this.pos.x < can_world.width - this.width) {
              this.pos.x -= dx / 30;
          } 
          if (mouse.y != this.pos.y && this.pos.y < can_world.height - this.height) {
              this.pos.y -= dy / 30;
...
function loop() {
    ...
    // ctx_vPort.drawImage(can_world, (sprites.player.pos.x < viewPort.width/2 || sprites.player.pos.x > can_world.width - viewPort.width/2) ? 0 : (sprites['player'].pos.x + (sprites['player'].width / 2) - viewPort.width/2), (sprites.player.pos.y < viewPort.height/2 || sprites.player.pos.y > can_world.height - viewPort.height/2) ? 0 : (sprites['player'].pos.y + (sprites['player'].height / 2) - viewPort.width/2), viewPort.width, viewPort.height, 0, 0, viewPort.width, viewPort.height); /* This is the 'viewport' itself btw. Just read it ou loud, helps when you want to understand what's going on */
    ...
    console.log('player x,y',sprites.player.pos.x,sprites.player.pos.y);
...
can_pCanvas.addEventListener("mousedown", (event) => {
    mouse.click = true;
    // mouse.x = event.x;
    // mouse.y = event.y;
  ...
  can_pCanvas.addEventListener("mouseup", (event) => {
    mouse.click = false; //false bhayo hane chai line draw hudaina
  });

let canvasPosition = can_pCanvas.getBoundingClientRect();
